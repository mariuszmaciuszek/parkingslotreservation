...
public class ReservationAboutToStartManager implements ReminderService {
    ...
    @Override
    public void process() {
        final List<OutboxEvent> eventList =
                reservationRepository.findAllReservationAboutToStart(inMinutesPeriod).stream()
                        .map(r -> OutboxEvent.builder()
                                .reservationId(r.getId())
                                .eventType(OutboxEvent.OutboxEventType.RESERVATION_START_REMINDER)
                                .id(UUID.randomUUID())
                                .build())
                        .toList();
